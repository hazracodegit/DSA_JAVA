...
Given a **circular integer array** `nums` (i.e., the next element of the last element is the first element), return the **next greater element** for every element in the array.
...
- **1** → Since the array is circular, continue from the beginning. Next greater element is **2**.
...
- 🚀 This is an optimized Monotonic Stack solution.
